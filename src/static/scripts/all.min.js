...
var LAST_DAY=0,LAST_WEEK=1,LAST_MONTH=2,DATE=3,WOEID=4,LIMIT=50;var history=LAST_DAY,woeid=23424969,response;var datepickerState=0;document.ready=function(){jQuery("#datepicker").datepicker({dateFormat:"dd.mm.y",minDate:new Date(1373576400000),maxDate:new Date(),onSelect:function(a){setHistory(this,DATE);}});jQuery("#datepicker").datepicker("setDate",new Date());getTrends(history);};window.onerror=function(){_("trends").innerHTML="Ooops! Something went wrong!";};if(!window.XMLHttpRequest){XMLHttpRequest=function(){try{return new ActiveXObject("Msxml2.XMLHTTP.6.0");}catch(a){}try{return new ActiveXObject("Msxml2.XMLHTTP.3.0");}catch(a){}try{return new ActiveXObject("Msxml2.XMLHTTP");}catch(a){}try{return new ActiveXObject("Microsoft.XMLHTTP");}catch(a){}throw new Error("Could not find an XMLHttpRequest alternative.");};}function _(a){return document.getElementById(a);}function getTrends(b){var c=document.URL.split("/");var a=c[0]+"//"+c[2]+"/rpc?woeid="+woeid;if(history==DATE){startDate=Math.floor(jQuery("#datepicker").datepicker("getDate").getTime()/1000);endDate=startDate+86400;a+="&timestamp="+startDate+"&end_timestamp="+endDate;}else{a+="&history="+getHistoryText();}a+="&limit="+LIMIT;var d=new XMLHttpRequest();d.open("GET",a,true);d.onreadystatechange=function(){if(d.readyState==4&&d.status==200){onSuccess(d.responseText);}else{if(d.readyState==4&&d.status!=200){onFailure();}}if(d.readyState==4){_("l"+b).style.display="none";}};d.send(null);console.log(a);_("l"+b).style.display="inline-block";}function onSuccess(a){try{response=JSON.parse(a);}catch(b){response=a;}if(response==""){_("trends").innerHTML="YOU SHOULD ENTER VALID INPUT!";return;}if(response.trends.length==0){_("trends").innerHTML="NO RECORD! PLEASE SELECT A DATE AFTER JULY, 12.";return;}drawTrends();}function onFailure(){_("trends").innerHTML="YOU CRASHED IT!";}function drawTrends(){_("trends").innerHTML=null;var b=_("trends").offsetWidth,d=_("trends").offsetHeight,g=d3.format(",d");var a=d3.layout.pack().sort(null).size([b,d]);var f=d3.select("#trends").append("svg").attr("width",b).attr("height",d).attr("class","bubble");var c=f.append("svg:defs").append("svg:filter").attr("id","dropshadow").attr("height","130%");c.append("svg:feGaussianBlur").attr("in","SourceAlpha").attr("stdDeviation",2);c.append("svg:feOffset").attr("dx",0).attr("dy",2).attr("result","offsetblur");var i=c.append("svg:feMerge");i.append("feMergeNode");i.append("feMergeNode").attr("in","SourceGraphic");var e=f.selectAll("g.node").data(a.nodes({children:response.trends}).filter(function(h){return !h.children;})).enter().append("g").attr("class","node").attr("transform",function(h){return"translate("+h.x+","+h.y+")";});e.append("circle").attr("r",function(h){return h.r;}).on("mouseover",function(){d3.select(this).style("filter","url(#dropshadow)");}).on("mouseout",function(){d3.select(this).style("filter","");});$("circle").tipsy({gravity:"s",html:true,fade:true,offset:30,title:function(){var h=this.__data__;return'<div class="tipsy-topic">'+h.name+'</div><span class="tipsy-time">'+h.value+" min.</span>";}});}function setHistory(b,a){if(history!=a||a==DATE){history=a;getTrends(history);changeHistoryBtnStyle(b,a);}return false;}function setWoeid(b,a){if(woeid!=a){woeid=a;getTrends(WOEID);changeRegionBtnStyle(b);}return false;}function setDateText(a){if(a){_("dateText").innerHTML=jQuery("#datepicker").datepicker("getDate").toDateString();}else{_("dateText").innerHTML="select a date";}}function changeRegionBtnStyle(a){$("nav a").removeClass("current");$(a).addClass("current");}function changeHistoryBtnStyle(b,a){if(a==DATE){setDateText(true);toggleDatepicker();$(".btn").removeClass("active");$("#datepickerBtn").addClass("active");}else{if(b){datepickerState=true;toggleDatepicker();setDateText(null);$(".btn").removeClass("active");$("#datepickerBtn").removeClass("datepicker-open");$(b).addClass("active");}}}function toggleDatepicker(){if(datepickerState){$("#datepicker").slideUp();$("#datepickerBtn").removeClass("datepicker-open");}else{$("#datepicker").slideDown();$("#datepickerBtn").addClass("datepicker-open");}datepickerState=!datepickerState;return false;}function getHistoryText(){if(history==LAST_DAY){return"ld";}else{if(history==LAST_WEEK){return"lw";}else{if(history==LAST_MONTH){return"lm";}}}}
