...
(function(){console.log("test");var t=0,q=1,r=2,h=3,a=4,o=50;var s=t,p=23424969,c;var n=0,A=0,g=null;$("a, .btn").on("click",function(){ga("send","event","link","click",$(this).attr("id")||$(this).attr("class"));});window.onload=function(){console.log("ready");$("#datepicker").datepicker({dateFormat:"dd.mm.y",minDate:new Date(1373576400000),maxDate:new Date(),onSelect:function(F){f(this,h);}});$("#datepicker").datepicker("setDate",new Date());d();E();d3.selection.prototype.moveToFront=function(){return this.each(function(){this.parentNode.appendChild(this);});};};window.onerror=function(){C("trends").innerHTML="Ooops! Something went wrong!";};if(!window.XMLHttpRequest){XMLHttpRequest=function(){try{return new ActiveXObject("Msxml2.XMLHTTP.6.0");}catch(F){}try{return new ActiveXObject("Msxml2.XMLHTTP.3.0");}catch(F){}try{return new ActiveXObject("Msxml2.XMLHTTP");}catch(F){}try{return new ActiveXObject("Microsoft.XMLHTTP");}catch(F){}throw new Error("Could not find an XMLHttpRequest alternative.");};}function C(F){return document.getElementById(F);}function E(){var G=document.URL.split("/");var F=G[0]+"//"+G[2]+"/rpc?woeid="+p;if(s==h){startDate=Math.floor(jQuery("#datepicker").datepicker("getDate").getTime()/1000);endDate=startDate+86400;F+="&timestamp="+startDate+"&end_timestamp="+endDate;}else{F+="&history="+e();}F+="&limit="+o;var H=new XMLHttpRequest();H.open("GET",F,true);H.onreadystatechange=function(){if(H.readyState==4){j();if(H.status==200){i(H.responseText);}else{z();}}};H.send(null);d();}function i(F){try{c=JSON.parse(F);}catch(G){c=F;}if(c==""){C("trends").innerHTML="YOU SHOULD ENTER VALID INPUT!";return;}if(c.trends.length==0){C("trends").innerHTML="NO RECORD! PLEASE PICK A DATE AFTER JULY, 12 2013.";return;}b();l();setTimeout(D,3000);}function z(){C("trends").innerHTML="YOU CRASHED IT!";}function d(){$("#trends").empty().append('<span>Loading...</span><div id="loading-area"><div class="spinner"></div></div>');}function j(){$("#trends").empty();}function b(F){k.draw(F);}function B(F){k.remove(F);}function D(G,F){if(A){return;}if(!G){G=$("circle");}if(typeof F=="undefined"){F=0;}if(G[F]){g=$(G[F]).mouseover();setTimeout(function(){$(G[F]).mouseout();g=null;D(G,F+1);},5000);}}function l(F){var F="Trending topics";if(s==t){F+=" within last 24 hours";}else{if(s==q){F+=" within last week";}else{if(s==r){F+=" within last month";}else{F+=" on "+jQuery("#datepicker").datepicker("getDate").toDateString().substring(4);}}}if(p==1){F+=" in World";}else{F+=" in Turkey";}$("#trends").prepend("<span>"+F+"</span>");}function f(G,F){if(s!=F||F==h){s=F;B(E);m(G,F);}return false;}function y(G,F){if(p!=F){p=F;B(E);v(G);}return false;}function u(F){if(F){C("dateText").innerHTML=jQuery("#datepicker").datepicker("getDate").toDateString().substring(4);}else{C("dateText").innerHTML="pick a date";}}function v(F){$("nav a").removeClass("current");$(F).addClass("current");}function m(G,F){if(F==h){u(true);w();$(".btn").removeClass("active");$("#datepickerBtn").addClass("active");}else{if(G){n=true;w();u(null);$(".btn").removeClass("active");$("#datepickerBtn").removeClass("datepicker-open");$(G).addClass("active");}}}function w(){if(n){$("#datepicker").slideUp();$("#datepickerBtn").removeClass("datepicker-open");}else{$("#datepicker").slideDown();$("#datepickerBtn").addClass("datepicker-open");}n=!n;return false;}function e(){if(s==t){return"ld";}else{if(s==q){return"lw";}else{if(s==r){return"lm";}}}}var x={bubbleChart:function(){var H=C("trends"),I,G,F;return{draw:function(){H.innerHTML=null;var R=H.offsetWidth,O=H.offsetHeight;var K=c.trends,J=K[0].value,S=d3.scale.category10(),L=(1000/R)*(J/100);I=d3.layout.force().gravity(0.03).charge(Q).nodes(K).size([R,O]).on("tick",P).start();F=d3.select("#trends").append("svg").attr("width",R).attr("height",O);G=F.selectAll(".node").data(K).enter().append("circle").attr("class","node").attr("cx",function(T){return T.x;}).attr("cy",function(T){return T.y;}).attr("r",0).style("fill",function(T){return M(T);}).style("stroke",function(U,T){return d3.rgb(U.color).darker(2);}).call(I.drag);G.transition().duration(1000).attr("r",function(T){return T.value/L;});F.style("opacity",0.000001).transition().duration(1000).style("opacity",1);$("circle").tipsy({gravity:"s",html:true,fade:false,offset:0,fadeAnimationDuration:200,title:function(){if(g){g.mouseout();A=1;g=null;}var T=d3.select(this);T.moveToFront();var U=this.__data__;return'<div class="tipsy-topic">'+U.name+'</div><span class="tipsy-time">'+N(U.value)+"</span>";}});function P(U){var T=-0.1*U.alpha;K.forEach(function(W,V){W.y+=T;W.x+=T;});G.attr("cx",function(V){return V.x;}).attr("cy",function(V){return V.y;});}function Q(T){return -Math.pow(T.value/(L*2),2)/8;}function M(T){T.color=S(Math.floor(T.value/(J/3)));return T.color;}function N(T){if(T==0){return"";}else{if(T>59){return Math.floor(T/60)+" h. "+N(T%60);}else{return T+" min.";}}}},remove:function(J){I.gravity(0.001).resume();G.transition().duration(1000).attr("r",0).remove();F.style("opacity",1).transition().duration(1000).style("opacity",0.000001).remove().each("end",J);}};}};var k=x.bubbleChart();jQuery("#trRegionBtn").click(function(){y(this,23424969);});jQuery("#worldRegionBtn").click(function(){y(this,1);});jQuery("#lastDayBtn").click(function(){f(this,t);});jQuery("#lastWeekBtn").click(function(){f(this,q);});jQuery("#lastMonthBtn").click(function(){f(this,r);});jQuery("#datepickerBtn").click(function(){w();});})();
