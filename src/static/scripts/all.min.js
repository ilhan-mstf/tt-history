...
(function(){var u=0,r=1,s=2,h=3,a=4,p=50;var t=u,q=1,c;var o=0,B=0,g=null;$("a, .btn").on("click",function(){ga("send","event","link","click",$(this).attr("id")||$(this).attr("class"));});window.onload=function(){$("#datepicker").datepicker({dateFormat:"dd.mm.y",minDate:new Date(1373576400000),maxDate:new Date(),onSelect:function(G){f(this,h);}});$("#datepicker").datepicker("setDate",new Date());$(".coming-soon").tipsy({gravity:"e"});d();F();d3.selection.prototype.moveToFront=function(){return this.each(function(){this.parentNode.appendChild(this);});};};window.onerror=function(){D("trends").innerHTML="Ooops! Something went wrong!";};if(!window.XMLHttpRequest){XMLHttpRequest=function(){try{return new ActiveXObject("Msxml2.XMLHTTP.6.0");}catch(G){}try{return new ActiveXObject("Msxml2.XMLHTTP.3.0");}catch(G){}try{return new ActiveXObject("Msxml2.XMLHTTP");}catch(G){}try{return new ActiveXObject("Microsoft.XMLHTTP");}catch(G){}throw new Error("Could not find an XMLHttpRequest alternative.");};}function D(G){return document.getElementById(G);}function F(){var H=document.URL.split("/");var G=H[0]+"//"+H[2]+"/rpc?woeid="+q;if(t==h){startDate=Math.floor(jQuery("#datepicker").datepicker("getDate").getTime()/1000);endDate=startDate+86400;G+="&timestamp="+startDate+"&end_timestamp="+endDate;}else{G+="&history="+e();}G+="&limit="+p;var I=new XMLHttpRequest();I.open("GET",G,true);I.onreadystatechange=function(){if(I.readyState==4){j();if(I.status==200){i(I.responseText);}else{if(I.status==503){A("Request limit exceeded. Try tomorrow! Or get in touch with me via @mustilica.");}else{A();}}}};I.send(null);m();d();}function i(G){try{c=JSON.parse(G);}catch(H){c=G;}if(c==""){D("trends").innerHTML="You should enter a valid input!";return;}if(c.trends.length==0){D("trends").innerHTML="No record! Please pick a date after July, 12 2013.";return;}b();l();B=0;setTimeout(E,3000);}function A(G){D("trends").innerHTML=G||"Ooops! Something went wrong!";}function d(){$("#trends").empty().append('<span>Loading...</span><div id="loading-area"><div class="spinner"></div></div>');}function j(){$("#trends").empty();}function b(G){k.draw(G);}function C(G){k.remove(G);}function E(H,G){try{if(B){return;}if(!H){H=$("circle");}if(typeof G=="undefined"){G=0;}if(H[G]){g=$(H[G]).mouseover();setTimeout(function(){$(H[G]).mouseout();g=null;E(H,G+1);},5000);}}catch(I){}}function m(){if(g){g.mouseout();B=1;g=null;}}function l(G){var G="Trending topics";if(t==u){G+=" within last 24 hours";}else{if(t==r){G+=" within last week";}else{if(t==s){G+=" within last month";}else{G+=" on "+jQuery("#datepicker").datepicker("getDate").toDateString().substring(4);}}}if(q==1){G+=" in Worldwide";}else{G+=" in Turkey";}$("#trends").prepend("<span>"+G+"</span>");}function f(H,G){if(t!=G||G==h){t=G;C(F);n(H,G);}return false;}function z(H,G){if(q!=G){q=G;C(F);w(H);}return false;}function v(G){if(G){D("dateText").innerHTML=jQuery("#datepicker").datepicker("getDate").toDateString().substring(4);}else{D("dateText").innerHTML="pick a date";}}function w(G){$("nav a").removeClass("current");$(G).addClass("current");}function n(H,G){if(G==h){v(true);x();$(".btn").removeClass("active");$("#datepickerBtn").addClass("active");}else{if(H){o=true;x();v(null);$(".btn").removeClass("active");$("#datepickerBtn").removeClass("datepicker-open");$(H).addClass("active");}}}function x(){if(o){$("#datepicker").slideUp();$("#datepickerBtn").removeClass("datepicker-open");}else{$("#datepicker").slideDown();$("#datepickerBtn").addClass("datepicker-open");}o=!o;return false;}function e(){if(t==u){return"ld";}else{if(t==r){return"lw";}else{if(t==s){return"lm";}}}}var y={bubbleChart:function(){var I=D("trends"),J,H,G;return{draw:function(){I.innerHTML=null;var S=I.offsetWidth,P=I.offsetHeight;var L=c.trends,K=L[0].duration,T=d3.scale.ordinal().range(Math.random()>=0.5?["#bd0026","#f03b20","#fd8d3c","#fecc5c","#ffffb2"]:["#253494","#2c7fb8","#41b6c4","#a1dab4","#ffffcc"]),M=(1000/S)*(K/50);J=d3.layout.force().gravity(0.03).charge(R).nodes(L).size([S,P]).on("tick",Q).start();G=d3.select("#trends").append("svg").attr("width",S).attr("height",P);H=G.selectAll(".node").data(L).enter().append("circle").attr("class","node").attr("cx",function(U){return U.x;}).attr("cy",function(U){return U.y;}).attr("r",0).style("fill",function(U){return N(U);}).style("stroke",function(V,U){return d3.rgb(V.color).darker(2);}).call(J.drag);H.transition().duration(1000).attr("r",function(U){return U.duration/M;});G.style("opacity",0.000001).transition().duration(1000).style("opacity",1);$("circle").tipsy({gravity:"s",html:true,fade:false,offset:0,fadeAnimationDuration:200,title:function(){m();var U=d3.select(this);U.moveToFront();var V=this.__data__;return'<div class="tipsy-topic">'+V.name+'</div><span class="tipsy-time">'+O(V.duration)+"</span>";}});function Q(V){var U=-0.1*V.alpha;L.forEach(function(X,W){X.y+=U;X.x+=U;});H.attr("cx",function(W){return W.x;}).attr("cy",function(W){return W.y;});}function R(U){return -Math.pow(U.duration/(M*2),2)/8;}function N(U){U.color=T(Math.floor(U.duration/(K/5)));return U.color;}function O(U){if(U==0){return"";}else{if(U>59){return Math.floor(U/60)+" h. "+O(U%60);}else{return U+" min.";}}}},remove:function(K){J.gravity(0.001).resume();H.transition().duration(1000).attr("r",0).remove();G.style("opacity",1).transition().duration(1000).style("opacity",0.000001).remove().each("end",K);}};}};var k=y.bubbleChart();jQuery("#trRegionBtn").click(function(){z(this,23424969);});jQuery("#worldRegionBtn").click(function(){z(this,1);});jQuery("#lastDayBtn").click(function(){f(this,u);});jQuery("#lastWeekBtn").click(function(){f(this,r);});jQuery("#lastMonthBtn").click(function(){f(this,s);});jQuery("#datepickerBtn").click(function(){x();});})();
