...
/*!
 * Generated using the Bootstrap Customizer (http://getbootstrap.com/customize/?id=8dcb476a93d73c3de3564069682bb493)
 * Config saved to config.json and https://gist.github.com/8dcb476a93d73c3de3564069682bb493
 */
...
(function(){var v=0,s=1,t=2,g=3,a=4,q=50;var u=v,r=1,c;var o=0,C=0,p=null;$("a, .btn").on("click",function(){ga("send","event","link","click",$(this).attr("id")||$(this).attr("class"));});$(document).ready(function(){var G=$("#trends");if($(window).width()<$(window).height()){G.height($(window).height()-200);}else{G.height($(window).height()-60);}m();});$(window).on("load",function(){$('[data-toggle="tooltip"]').tooltip();d();F();d3.selection.prototype.moveToFront=function(){return this.each(function(){this.parentNode.appendChild(this);});};});$(window).on("error",function(){$("#trends").html("Ooops! Something went wrong!");});$(window).on("resize",m);function m(){if($(window).width()<992){$("#controls").removeClass("pull-right text-right").addClass("list-inline");}else{$("#controls").removeClass("list-inline").addClass("pull-right text-right");}}function F(){var H=document.URL.split("/");var G=H[0]+"//"+H[2]+"/rpc?woeid="+r;if(u==g){startDate=Math.floor($("#datepicker").datepicker("getDate").getTime()/1000);endDate=startDate+86400;G+="&timestamp="+startDate+"&end_timestamp="+endDate;}else{G+="&history="+e();}G+="&limit="+q;var I=new XMLHttpRequest();I.open("GET",G,true);I.onreadystatechange=function(){if(I.readyState==4){i();if(I.status==200){h(I.responseText);}else{if(I.status==503){B('Request limit exceeded. Try tomorrow! Or get in touch with me via <a href="https://twitter.com/#!/mustilica">@mustilica</a>.');$("#controls").hide();}else{B();}}}};I.send(null);l();d();}function h(G){try{c=JSON.parse(G);}catch(H){c=G;}if(c==""){$("#trends").html("You should enter a valid input!");return;}if(c.trends.length==0){$("#trends").html("No record! Please pick a date after July, 12 2013.");return;}b();k();C=0;setTimeout(E,3000);}function B(G){$("#trends").html(G||"Ooops! Something went wrong!");}function d(){$("#trends").empty().append('<span>Loading...</span><div id="loading-area"><div class="spinner"></div></div>');}function i(){$("#trends").empty();}function b(G){j.draw(G);}function D(G){j.remove(G);}function E(H,G){try{if(C){return;}if(!H){H=$("circle");}if(typeof G=="undefined"){G=0;}if(H[G]){p=$(H[G]).mouseover();setTimeout(function(){$(H[G]).mouseout();p=null;E(H,G+1);},5000);}}catch(I){}}function l(){if(p){p.mouseout();C=1;p=null;}}function k(G){var G="Trending topics";if(u==v){G+=" within last 24 hours";}else{if(u==s){G+=" within last week";}else{if(u==t){G+=" within last month";}else{G+=" on "+$("#datepicker").datepicker("getDate").toDateString().substring(4);}}}if(r==1){G+=" in Worldwide";}else{G+=" in Turkey";}$("#trends").prepend("<span>"+G+"</span>");}function f(H,G){if(u!=G||G==g){u=G;D(F);n(H,G);}return false;}function A(H,G){if(r!=G){r=G;D(F);x(H);}return false;}function w(G){if(G){$("#dateText").html($("#datepicker")).datepicker("getDate").toDateString().substring(4);}else{$("#dateText").html("pick a date");}}function x(G){$(".country ").removeClass("current");G.parent().addClass("current");}function n(H,G){if(G==g){w(true);y();$(".btn").removeClass("active");$("#datepickerBtn").addClass("active");}else{if(H){o=true;y();w(null);$(".btn").removeClass("active");$("#datepickerBtn").removeClass("datepicker-open");$(H).addClass("active");}}}function y(){if(o){$("#datepicker").slideUp();$("#datepickerBtn").removeClass("datepicker-open");}else{$("#datepicker").slideDown();$("#datepickerBtn").addClass("datepicker-open");}o=!o;return false;}function e(){if(u==v){return"ld";}else{if(u==s){return"lw";}else{if(u==t){return"lm";}}}}var z={bubbleChart:function(){var I=$("#trends"),J,H,G;return{draw:function(){I.html("");var S=I.outerWidth(),P=I.outerHeight();var L=c.trends,K=L[0].duration,T=d3.scale.ordinal().range(["#fdea6f","#f3b355","#e97e3b","#cf4f29","#723c2c"]),M=(1000/S)*(K/50);J=d3.layout.force().gravity(0.03).charge(R).nodes(L).size([S,P]).on("tick",Q).start();G=d3.select("#trends").append("svg").attr("width",S).attr("height",P);H=G.selectAll(".node").data(L).enter().append("circle").attr("class","node").attr("cx",function(U){return U.x;}).attr("cy",function(U){return U.y;}).attr("r",0).style("fill",function(U){return N(U);}).style("stroke",function(V,U){return d3.rgb(V.color).darker(2);}).call(J.drag);H.transition().duration(1000).attr("r",function(U){return U.duration/M;});G.style("opacity",0.000001).transition().duration(1000).style("opacity",1);$("circle").tooltip({container:"body",animation:false,html:true,title:function(){var U=d3.select(this);return'<div class="tooltip-topic">'+U[0][0].__data__.name+'</div><div class="tooltip-time">'+O(U[0][0].__data__.duration)+"</div>";}});$("circle").on("hide.bs.tooltip",function(){d3.select(this).transition().duration(200).style("opacity",1).attr("r",function(U){return U.duration/M;});});$("circle").on("shown.bs.tooltip",function(){var V=$(document).scrollTop();if(V>0){var U=$(".tooltip");U.css({top:U.position().top+V});}l();d3.select(this).moveToFront().transition().duration(200).style("opacity",0.9).attr("r",function(W){return W.duration/M+10;});});function Q(V){var U=-0.1*V.alpha;L.forEach(function(X,W){X.y+=U;X.x+=U;});H.attr("cx",function(W){return W.x;}).attr("cy",function(W){return W.y;});}function R(U){return -Math.pow(U.duration/(M*2),2)/8;}function N(U){U.color=T(Math.floor(U.duration/(K/5)));return U.color;}function O(U){if(U==0){return"";}else{if(U>59){return Math.floor(U/60)+" h. "+O(U%60);}else{return U+" min.";}}}},remove:function(K){J.gravity(0.001).resume();H.transition().duration(1000).attr("r",0).remove();G.style("opacity",1).transition().duration(1000).style("opacity",0.000001).remove().each("end",K);}};}};var j=z.bubbleChart();$(".region").click(function(){var G=$(this).attr("data-woeid");if(G!=""){A($(this),$(this).attr("data-woeid"));}});$("#datepickerBtn").click(function(){y();});})();
