...
(function(){var r=0,o=1,p=2,g=3,a=4,m=50;var q=r,n=23424969,b;var l=0;document.ready=function(){jQuery("#datepicker").datepicker({dateFormat:"dd.mm.y",minDate:new Date(1373576400000),maxDate:new Date(),onSelect:function(B){f(this,g);}});jQuery("#datepicker").datepicker("setDate",new Date());d();A();};window.onerror=function(){z("trends").innerHTML="Ooops! Something went wrong!";};if(!window.XMLHttpRequest){XMLHttpRequest=function(){try{return new ActiveXObject("Msxml2.XMLHTTP.6.0");}catch(B){}try{return new ActiveXObject("Msxml2.XMLHTTP.3.0");}catch(B){}try{return new ActiveXObject("Msxml2.XMLHTTP");}catch(B){}try{return new ActiveXObject("Microsoft.XMLHTTP");}catch(B){}throw new Error("Could not find an XMLHttpRequest alternative.");};}function z(B){return document.getElementById(B);}function A(){var C=document.URL.split("/");var B=C[0]+"//"+C[2]+"/rpc?woeid="+n;if(q==g){startDate=Math.floor(jQuery("#datepicker").datepicker("getDate").getTime()/1000);endDate=startDate+86400;B+="&timestamp="+startDate+"&end_timestamp="+endDate;}else{B+="&history="+e();}B+="&limit="+m;var D=new XMLHttpRequest();D.open("GET",B,true);D.onreadystatechange=function(){if(D.readyState==4){if(D.status==200){h(D.responseText);}else{x();}i();}};D.send(null);console.log(B);d();}function h(B){try{b=JSON.parse(B);}catch(C){b=B;}if(b==""){z("trends").innerHTML="YOU SHOULD ENTER VALID INPUT!";return;}if(b.trends.length==0){z("trends").innerHTML="NO RECORD! PLEASE SELECT A DATE AFTER JULY, 12 2013.";return;}c();}function x(){z("trends").innerHTML="YOU CRASHED IT!";}function d(){jQuery("#trends").empty().append('<div id="loading-area"><div class="spinner"></div></div>');}function i(){jQuery("#loading-area").remove();}function c(B){j.draw(B);}function y(B){j.remove(B);}function f(C,B){if(q!=B||B==g){q=B;y(A);k(C,B);}return false;}function w(C,B){if(n!=B){n=B;y(A);t(C);}return false;}function s(B){if(B){z("dateText").innerHTML=jQuery("#datepicker").datepicker("getDate").toDateString().substring(4);}else{z("dateText").innerHTML="select a date";}}function t(B){$("nav a").removeClass("current");$(B).addClass("current");}function k(C,B){if(B==g){s(true);u();$(".btn").removeClass("active");$("#datepickerBtn").addClass("active");}else{if(C){l=true;u();s(null);$(".btn").removeClass("active");$("#datepickerBtn").removeClass("datepicker-open");$(C).addClass("active");}}}function u(){if(l){$("#datepicker").slideUp();$("#datepickerBtn").removeClass("datepicker-open");}else{$("#datepicker").slideDown();$("#datepickerBtn").addClass("datepicker-open");}l=!l;return false;}function e(){if(q==r){return"ld";}else{if(q==o){return"lw";}else{if(q==p){return"lm";}}}}var v={bubbleChart:function(){var D=z("trends"),E,C,B;return{draw:function(){D.innerHTML=null;var N=D.offsetWidth,K=D.offsetHeight;var G=b.trends,F=G[0].value,O=d3.scale.category10(),H=(1000/N)*(F/100);E=d3.layout.force().gravity(0.03).charge(M).nodes(G).size([N,K]).on("tick",L).start();B=d3.select("#trends").append("svg").attr("width",N).attr("height",K);C=B.selectAll(".node").data(G).enter().append("circle").attr("class","node").attr("cx",function(P){return P.x;}).attr("cy",function(P){return P.y;}).attr("r",0).style("fill",function(P){return I(P);}).style("stroke",function(Q,P){return d3.rgb(Q.color).darker(2);}).call(E.drag);C.transition().duration(1000).attr("r",function(P){return P.value/H;});B.style("opacity",0.000001).transition().duration(1000).style("opacity",1);$("circle").tipsy({gravity:"s",html:true,fade:false,offset:30,fadeAnimationDuration:200,title:function(){var P=this.__data__;return'<div class="tipsy-topic">'+P.name+'</div><span class="tipsy-time">'+J(P.value)+"</span>";}});function L(Q){var P=-0.1*Q.alpha;G.forEach(function(S,R){S.y+=P;S.x+=P;});C.attr("cx",function(R){return R.x;}).attr("cy",function(R){return R.y;});}function M(P){return -Math.pow(P.value/(H*2),2)/8;}function I(P){P.color=O(Math.floor(P.value/(F/3)));return P.color;}function J(P){if(P==0){return"";}else{if(P>59){return Math.floor(P/60)+" h. "+J(P%60);}else{return P+" min.";}}}},remove:function(F){E.gravity(0.001).resume();C.transition().duration(1000).attr("r",0).remove();B.style("opacity",1).transition().duration(1000).style("opacity",0.000001).remove().each("end",F);}};}};var j=v.bubbleChart();jQuery("#trRegionBtn").click(function(){w(this,23424969);});jQuery("#worldRegionBtn").click(function(){w(this,1);});jQuery("#lastDayBtn").click(function(){f(this,r);});jQuery("#lastWeekBtn").click(function(){f(this,o);});jQuery("#lastMonthBtn").click(function(){f(this,p);});jQuery("#datepickerBtn").click(function(){u();});})();
